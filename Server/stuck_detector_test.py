...
class TestStuckDetector(unittest.TestCase):
    ...
    def test_invalid_history_type(self):
        with self.assertRaises(TypeError):
            self.detector.detect_stuck("not a list")

        with self.assertRaises(TypeError):
            self.detector.detect_stuck(None)

    def test_invalid_run_shape(self):
        with self.assertRaises(TypeError):
            self.detector.detect_stuck([1, 2, 3])

        with self.assertRaises(ValueError):
            self.detector.detect_stuck([{'params': {'p': 1}}])

        with self.assertRaises(ValueError):
            self.detector.detect_stuck([{'speed_mps': 1.0}])

    def test_invalid_speed_values(self):
        with self.assertRaises(TypeError):
            self.detector.detect_stuck([self.create_run("fast", {'p': 1})])

        with self.assertRaises(ValueError):
            self.detector.detect_stuck([self.create_run(float("nan"), {'p': 1})])

        with self.assertRaises(ValueError):
            self.detector.detect_stuck([self.create_run(float("inf"), {'p': 1})])

        with self.assertRaises(ValueError):
            self.detector.detect_stuck([self.create_run(-0.1, {'p': 1})])

    def test_invalid_params_values(self):
        with self.assertRaises(TypeError):
            self.detector.detect_stuck([self.create_run(1.0, "not a dict")])

        with self.assertRaises(ValueError):
            self.detector.detect_stuck([self.create_run(1.0, {})])

        with self.assertRaises(TypeError):
            self.detector.detect_stuck([self.create_run(1.0, {'p': "x"})])

        with self.assertRaises(ValueError):
            self.detector.detect_stuck([self.create_run(1.0, {'p': float("nan")})])

    def test_invalid_flags(self):
        run = self.create_run(1.0, {'p': 1})
        run['overcurrent'] = "yes"
        with self.assertRaises(TypeError):
            self.detector.detect_stuck([run])
